...
# Only these get a color prefix
COLOR_CLASSES = {"car", "truck", "bus", "motorcycle", "bicycle"}

# Loiter only for people (recommended)
        ...
        while not stop_flag["stop"]:
            ...
            if r.boxes is not None and len(r.boxes) > 0:
                for box in r.boxes:
                    ...
                    # Only vehicles get a color prefix
                    color_prefix = ""
...
# -----------------------
def main():
    # accuracy > speed (still pretty fast)
    model = YOLO("yolov8s.pt")   # try yolov8m.pt if you want even better
    ...
    cv2.namedWindow("Cyrelo - Step 5 (Precise + Vehicle Color)", cv2.WINDOW_NORMAL)
    ...
    while True:
        ...
        if not q_out.empty():
            ...
            cv2.imshow("Cyrelo - Step 5 (Precise + Vehicle Color)", frame)
