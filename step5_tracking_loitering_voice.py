...
# Voice (offline, Windows-friendly)
import pyttsx3
...
# Only vehicles get color prefix
COLOR_CLASSES = {"car", "truck", "bus", "motorcycle", "bicycle"}
...
# Prevent voice spam (extra layer, besides per-track cooldown)
GLOBAL_VOICE_COOLDOWN_SEC = 2.0
...
# -----------------------
# COLOR ESTIMATION (vehicles only)
# -----------------------
def estimate_vehicle_color(frame_bgr, x1, y1, x2, y2):
    ...
    # inner crop to reduce background
    rh, rw = roi.shape[:2]
...
# -----------------------
# VOICE THREAD (non-blocking)
# -----------------------
def voice_thread_fn(q_speech, stop_flag):
...
def tracker_loiter_thread(model, q_frames, q_out, q_speech, stop_flag):
    inside_since = {}
    last_seen_inside = {}
    last_event_time = {}

    last_global_voice = 0.0

    with open(CSV_PATH, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["timestamp", "event_type", "label", "confidence", "track_id", "zone"])
        ...
        while not stop_flag["stop"]:
            ...
            if r.boxes is not None and len(r.boxes) > 0:
                for box in r.boxes:
                    ...
                    # vehicle color prefix only
                    color_prefix = ""
                    if label in COLOR_CLASSES:
                        c = estimate_vehicle_color(frame, x1, y1, x2, y2)
                        if c != "unknown":
                            color_prefix = f"{c} "
                    ...
                    if label in LOITER_CLASSES and track_id is not None:
                        if in_zone:
                            ...
                            if dwell >= LOITER_SECONDS and (now - last_evt) >= EVENT_COOLDOWN_SEC:
                                ...
                                log_event_csv(writer, "LOITERING", label, conf, track_id, ZONE_NAME)
                                f.flush()

                                # Voice alert (non-blocking)
                                if VOICE_ENABLED and (now - last_global_voice) >= GLOBAL_VOICE_COOLDOWN_SEC:
                                    last_global_voice = now
                                    msg = f"Warning. Person loitering in {ZONE_NAME}."
                                    try:
                                        if q_speech.full():
                                            q_speech.get_nowait()
                                    except Empty:
                    ...
                    text = f"{color_prefix}{label} {conf:.2f} {id_txt} {dwell_txt}".strip()
...
# -----------------------
def main():
    model = YOLO("yolov8s.pt")  # accuracy upgrade

    q_frames = Queue(maxsize=QUEUE_MAX)
    q_out = Queue(maxsize=QUEUE_MAX)
    q_speech = Queue(maxsize=3)

    stop_flag = {"stop": False}

    t_reader = threading.Thread(target=reader_thread, args=(q_frames, stop_flag), daemon=True)
    t_track = threading.Thread(target=tracker_loiter_thread, args=(model, q_frames, q_out, q_speech, stop_flag), daemon=True)
    t_voice = threading.Thread(target=voice_thread_fn, args=(q_speech, stop_flag), daemon=True)

    t_reader.start()
    t_track.start()
    t_voice.start()

    cv2.namedWindow("Cyrelo - Step 5 (Voice Alerts)", cv2.WINDOW_NORMAL)
    ...
    while True:
        ...
        if not q_out.empty():
            ...
            cv2.putText(
                frame,
                f"FPS: {display_fps:.1f} | Voice: {'ON' if VOICE_ENABLED else 'OFF'} | Q to quit",
                (15, 30),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.8,
                (255, 255, 255),
            ...
            cv2.imshow("Cyrelo - Step 5 (Voice Alerts)", frame)
